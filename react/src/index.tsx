...
function AdminCheckable(props: AdminCheckableProps, tag: "aui-checkbox" | "aui-switch") {
    ...
    useCustomEvent(
        ...
        "aui-checked-change",
        onCheckedChange
            ? (detail: { checked: boolean }) => onCheckedChange(detail.checked)
            : undefined,
    );
...
export function AdminStat(props: AdminStatProps) {
    const { element, setRef } = useAdminElement(undefined);
    const { className, label, value, unit, trend, tone, ...rest } = props;
    useElementProperties(element as RefObject<CustomElement | null>, {
        label,
        value,
        unit,
...
export type {
    ...
    AdminTreeTableProps,
    AdminListViewProps,
    AdminFilterBuilderProps,
    AdminQueryBuilderProps,
