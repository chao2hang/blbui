...
```bash
git tag blbui-v0.0.27
git push origin blbui-v0.0.27
```
...
metadata 中（默认最多等待 10 分钟，每次请求最多等待 15 秒）。这覆盖 npm metadata 的短暂传播延迟；
...
当前 package version 是 `0.0.27`。已经发布后不能重复发布同一个版本，需要先统一修改 workspace package 的 version，再创建新的 tag；每次版本同时更新 CHANGELOG、文档站版本标识和 peer dependency 范围。
