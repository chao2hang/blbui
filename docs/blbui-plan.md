...
> **状态（0.0.22 更新）**：Core 现有 **104 个组件**、Business 22 个、React/Vue 为 104:1 完整 Core 绑定、Svelte 提供注册入口、29 个常用封装（共 31 个公开导出）；9 套主题均支持 light/dark，并提供语义 CSS utilities。当前已接入 token lint、governance audit、Vitest、Playwright/a11y/E2E、文档 smoke、SSR 检查、四套 playground 构建与真实浏览器 parity、公共 API 文档检查、数据 adapter contract、chart adapter contract、Line/AreaChart 多系列、Heatmap/Funnel/Gantt、编辑器 adapter、外部宿主迁移示例、versioned parity fixture、Business parity fixture、docs 性能预算、visual matrix/golden 和人工视觉验收；文档站覆盖 126 个组件。异步数据/业务组件已统一 permission denied、retry 事件与具名插槽，四套 playground 已共用真实 `AdminDataResource` 生命周期，并新增有限重试/指数退避、请求缓存、stale-while-revalidate、缓存观测钩子和稳定的缓存事件统计，发布预检已接入 format/release gate；docs-site 已提供异步状态实验台和恢复交互回归，固定 runner golden 已通过完整 manifest 提升流程维护。
> 后续质量收口新增 Business Vue/Svelte 直接 Custom Elements 指南（22 个元素）、图表窄屏/主题 E2E 与 npm 发布 job summary；真实 `web/` 宿主迁移仍需宿主源码进入工作区后逐页执行。
...
- [x] 建立 `@chaos_team/blbui-svelte` 注册/类型入口与 29 个常用封装（共 31 个公开导出，新增 PageHeader/Shell 布局挂载）。
- [x] 建立 `@chaos_team/blbui-business`（22 组件）与 `@chaos_team/blbui-business-react`。
...
- [x] 发布 0.0.20 小版本：Business React 图表行为测试、四框架真实 `AdminDataResource` parity、Web Components pagehide 清理和全 workspace format release gate 纳入稳定流程。
- [x] 发布 0.0.21 小版本：`AdminDataResource` 重试退避、缓存/SWR、缓存失效和跨框架数据能力文档纳入稳定流程。
- [x] 发布 0.0.22 小版本：缓存观测/统计、Business Custom Elements 事件回归、Vue/Svelte 窄屏门禁、ImportDialog 降级和固定 runner golden 提升流程纳入稳定发布流程。
