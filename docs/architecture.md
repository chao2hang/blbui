...
- `catalog:check` 校验目录完整性（104 个 Core + 22 个 Business，共 126 个组件）与文档示例中每个 `Admin*` 导入、每个 `aui-*` 标签在对应包里真实存在。
...
- `docs/editor-adapters.md` 固化 CodeMirror、TipTap、Monaco 的可选映射，以及 React/Vue/Svelte/Web Components 的订阅清理生命周期。
