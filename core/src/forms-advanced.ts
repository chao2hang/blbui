...
export class AdminFieldElement extends AdminElement {
    ...
    error = "";
    required = false;
    private labelId = nextUid("field-label");
    private controlSlotChange(event: Event): void {
        ...
        if (target) target.setAttribute("aria-labelledby", this.labelId);
    }
    render() {
        return html`<div class="field">
            <label id=${this.labelId}
                >${this.label}<span class="required" ?hidden=${!this.required}> *</span></label
            >${this.description ? html`<div class="description">${this.description}</div>` : null}
            <div class="control"><slot @slotchange=${this.controlSlotChange}></slot></div>
