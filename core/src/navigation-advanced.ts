...
export class AdminAccordionElement extends AdminElement {
    ...
    private toggle(event: Event): void {
        ...
        if (!this.multiple && details.open) {
            for (const other of this.shadowRoot?.querySelectorAll<HTMLDetailsElement>("details") ??
                []) {
    ...
    render() {
        return html`<div class="accordion">
            ${this.items.map(
                (item) =>
                    html`<details ?disabled=${item.disabled} @toggle=${this.toggle}>
        ...
        return html`<div class="stepper" role="list">
            ${this.items.map((item, index) => {
                ...
                return html`<div class="step" data-state=${state} role="listitem">
                    <span class="number">${number}</span><span>${item.label}</span>${
...
export class AdminListElement extends AdminElement {
    ...
    private keydown(event: KeyboardEvent): void {
        if (!this.selectable || !this.items.length) return;
        ...
        const move = (delta: number) => {
            ...
            this.select(this.items[next].id);
        ...
        switch (event.key) {
            ...
            case "Home":
                ...
                break;
            case "End":
                this.select(this.items[this.items.length - 1].id);
                event.preventDefault();
                break;
        }
    }
    render() {
        return html`<div
            ...
            tabindex=${this.selectable ? 0 : undefined}
        ...
        >
            ${this.items.map((item) => {
                ...
                return html`<div
                    ...
                    tabindex=${this.selectable ? (selected ? 0 : -1) : undefined}
                    @click=${() => this.select(item.id)}
                >
...
export class AdminTreeElement extends AdminElement {
    ...
    private visibleNodes(): AdminTreeNode[] {
        const out: AdminTreeNode[] = [];
        const walk = (nodes: AdminTreeNode[]) => {
            for (const node of nodes) {
                out.push(node);
                if (node.children?.length && this.isExpanded(node.id)) walk(node.children);
            }
        };
        walk(this.nodes);
        return out;
    ...
    private keydown(event: KeyboardEvent): void {
        ...
        const current = visible.findIndex((node) => node.id === this.selected);
        const focusAt = (index: number) => {
            const clamped = Math.max(0, Math.min(visible.length - 1, index));
            this.select(visible[clamped].id);
            event.preventDefault();
        };
        switch (event.key) {
            case "ArrowDown":
            ...
            case "ArrowRight": {
                ...
                if (node?.children?.length) {
                    if (!this.isExpanded(node.id)) this.toggle(node.id);
            ...
            case "ArrowLeft": {
                const node = visible[current];
                if (node?.children?.length && this.isExpanded(node.id)) this.toggle(node.id);
                break;
            }
            case "Enter":
            case " ":
                if (visible[current]) {
                    this.select(visible[current].id);
                    event.preventDefault();
                }
                break;
        }
            ...
            (node) =>
                html`<div>
                    <div
                        ...
                        aria-selected=${node.id === this.selected ? "true" : "false"}
                        data-selected=${node.id === this.selected ? "true" : "false"}
                        tabindex=${node.id === this.selected ? 0 : -1}
    ...
    private depthOf(node: AdminTreeNode): number {
        const find = (nodes: AdminTreeNode[], depth: number): number => {
            for (const item of nodes) {
                if (item === node) return depth;
                if (item.children?.length) {
                    const found = find(item.children, depth + 1);
                    if (found > 0) return found;
                }
            }
            return 0;
        };
        return find(this.nodes, 1) || 1;
    }
    render() {
        return html`<div class="tree" role="tree" @keydown=${this.keydown}>
