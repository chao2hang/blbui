...
export class AdminToggleGroupElement extends AdminElement {
    ...
    private select(id: string): void {
        if (this.multiple) {
            this.values = this.values.includes(id)
                ? this.values.filter((item) => item !== id)
                : [...this.values, id];
            this.dispatchDetail("aui-toggle-group-change", { value: id, values: [...this.values] });
            this.requestUpdate();
            return;
        }
        this.value = id;
...
export class AdminAspectRatioElement extends AdminElement {
    ...
    ratio = "16 / 9";
    render() {
        return html`<div class="frame" style=${`--aui-aspect-ratio:${this.ratio}`}>
            <div class="content"><slot></slot></div>
        </div>`;
    }
}
...
export class AdminScrollAreaElement extends AdminElement {
    ...
    maxHeight = "320px";
    render() {
        return html`<div class="scroll" style=${`--aui-scroll-max-height:${this.maxHeight}`}>
            <slot></slot>
        </div>`;
    }
}
...
export class AdminNumberInputElement extends AdminElement {
    ...
    label = "Number";
    disabled = false;
    // Text currently in the input while the user is editing. Kept so that an
    // empty field does not snap to 0 (Number("") === 0) and partial input is
    ...
    private input(event: Event): void {
        ...
        if (!Number.isNaN(parsed)) this.updateValue(parsed);
    }
    protected updated(changed: Map<string, unknown>): void {
        if (changed.has("value")) this.draft = String(this.value);
    }
    render() {
...
export class AdminCodeBlockElement extends AdminElement {
    ...
    private async copy(): Promise<void> {
        try {
            ...
            // Clipboard can be denied or unavailable; still surface the intent.
        }
...
export class AdminColorTagElement extends AdminElement {
    ...
    static styles = css`
        ...
        .swatch {
            width: 13px;
            height: 13px;
            border: 1px solid var(--aui-white-20);
