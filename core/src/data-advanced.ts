...
export class AdminDataListElement extends AdminElement {
    ...
    static styles = css`
        ...
        .row {
            ...
            grid-template-columns: repeat(
                var(--aui-datalist-columns, 2),
                minmax(120px, 0.8fr) minmax(0, 1.5fr)
            );
            gap: 12px 20px;
            padding: 12px 0;
    ...
    columns = 2;
    render() {
        const perRow = Math.max(1, Math.min(this.columns, this.items.length || this.columns));
        const rows: Array<Array<{ label: string; value: string }>> = [];
        for (let index = 0; index < this.items.length; index += perRow) {
            rows.push(this.items.slice(index, index + perRow));
        }
        return html`<dl class="list" style=${`--aui-datalist-columns:${perRow}`}>
            ${rows.map(
                (row) =>
                    html`<div class="row">
                        ${row.map(
...
export class AdminCalendarGridElement extends AdminElement {
    ...
    connectedCallback(): void {
        ...
        const now = new Date();
        if (this.month < 0) this.month = now.getMonth();
    ...
    render() {
        ...
        return html`<div class="calendar">
            <div class="header">
                <button type="button" aria-label="Previous month" @click=${() => this.move(-1)}>
                    ‹</button
                ><strong>${this.year} / ${String(this.month + 1).padStart(2, "0")}</strong
                ><button type="button" aria-label="Next month" @click=${() => this.move(1)}>
                    ›
                </button>
            </div>
            <div class="grid">
                    ...
                    (item) =>
                        html`<button
                            ...
                            data-selected=${
                                !item.muted &&
                                this.selected ===
                                    `${this.year}-${String(this.month + 1).padStart(2, "0")}-${String(item.day).padStart(2, "0")}`
                                    ? "true"
                                    : "false"
                            }
                            ?disabled=${item.muted}
