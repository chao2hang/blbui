...
describe("expanded component contracts", () => {
    it("selects a leaf path from a cascader and emits the complete path", async () => {
        ...
        await cascader.updateComplete;
        const leaf = cascader.shadowRoot?.querySelectorAll<HTMLButtonElement>(
            ".level:last-child .option",
        )[0];
        leaf?.click();
        await cascader.updateComplete;
    ...
    it("moves selected transfer options between source and target lists", async () => {
        ...
        const sourceOption = transfer.shadowRoot?.querySelector<HTMLButtonElement>(
            ".list:first-child .option",
        );
        sourceOption?.click();
        await transfer.updateComplete;
    ...
    it("exposes hover-card trigger state and dismissible notifications", async () => {
        ...
        hover.innerHTML =
            '<button slot="trigger">Inspect</button><span slot="content">Details</span>';
    ...
    it("renders upload statuses and emits remove/retry/preview contracts", async () => {
        ...
        uploads.shadowRoot
            ?.querySelector<HTMLButtonElement>("article.item button[aria-label^='Remove']")
            ?.click();
        await uploads.updateComplete;
    ...
    it("opens file preview metadata and emits close/download events", async () => {
        ...
        expect(preview.shadowRoot?.querySelector(".file-name")?.textContent).toContain(
            "report.pdf",
        );
        let downloaded = "";
        let closed = false;
    ...
    it("keeps a progress value within its accessible range", async () => {
        ...
        expect(progress.shadowRoot?.querySelector<HTMLElement>(".indicator")?.style.width).toBe(
            "100%",
        );
    });
...
describe("accessibility contracts", () => {
    ...
    it("associates a field label with its slotted control", async () => {
        ...
        await (
            field.querySelector("aui-input") as unknown as {
                updateComplete: Promise<boolean>;
            }
        ).updateComplete;
...
describe("composed form and data contracts", () => {
    it("renders form items with label, description and invalid associations", async () => {
        const item = document.createElement("aui-form-item") as HTMLElement & {
            label: string;
            description: string;
            error: string;
            updateComplete: Promise<boolean>;
        ...
        item.label = "EMAIL";
        item.description = "Used for deployment alerts.";
        item.error = "Invalid address";
        item.innerHTML = "<input type='email' required />";
        ...
        expect(labelId).toBeTruthy();
        expect(item.shadowRoot?.querySelector(`label#${labelId}`)?.textContent).toContain("EMAIL");
        expect(input.getAttribute("aria-invalid")).toBe("true");
        expect(input.getAttribute("aria-describedby")).toBeTruthy();
    });

    it("emits schema form changes and validates a required field", async () => {
        const form = document.createElement("aui-schema-form") as AdminSchemaFormElement;
        form.fields = [
        ...
        form.addEventListener("aui-change", (event) => {
            change = (event as CustomEvent<{ name: string; value: unknown }>).detail;
        });
        form.addEventListener("aui-submit", (event) => {
            submit = (event as CustomEvent<{ valid: boolean }>).detail;
        });
        const input = form.shadowRoot
            ?.querySelector<HTMLElement>("aui-form-item")
            ?.querySelector<HTMLInputElement>("input[name='name']");
        expect(input).not.toBeNull();
        input!.value = "Gateway";
        input!.dispatchEvent(new Event("input", { bubbles: true }));
        await form.updateComplete;
        expect(change).toMatchObject({ name: "name", value: "Gateway" });

        form.shadowRoot
            ?.querySelector<HTMLFormElement>("form")
            ?.dispatchEvent(new SubmitEvent("submit", { bubbles: true, cancelable: true }));
        expect(submit?.valid).toBe(true);
    });

    it("supports DataGrid sorting, filtering, selection and batch actions", async () => {
        const grid = document.createElement("aui-data-grid") as AdminDataGridElement;
        grid.columns = [
        ...
        grid.rows = [
            { id: "a", name: "Alpha", status: "online" },
            { id: "b", name: "Beta", status: "offline" },
        ];
        grid.selectable = true;
        grid.batchActions = [{ id: "archive", label: "Archive" }];
        document.body.append(grid);
        await grid.updateComplete;
        ...
        let action: unknown;
        grid.addEventListener("aui-sort-change", (event) => (sort = (event as CustomEvent).detail));
        grid.addEventListener(
            "aui-filter-change",
            (event) => (filter = (event as CustomEvent).detail),
        );
        grid.addEventListener(
            "aui-selection-change",
            (event) => (selection = (event as CustomEvent).detail),
        );
        grid.addEventListener(
            "aui-batch-action",
            (event) => (action = (event as CustomEvent).detail),
        );

        grid.shadowRoot?.querySelector<HTMLButtonElement>("button.sort")?.click();
        await grid.updateComplete;
        expect(sort).toEqual({ key: "name", direction: "asc" });

        const nameFilter = grid.shadowRoot?.querySelector<HTMLInputElement>("input.filter");
        nameFilter!.value = "Beta";
        nameFilter!.dispatchEvent(new Event("input", { bubbles: true }));
        await grid.updateComplete;
        expect(filter).toMatchObject({ key: "name", value: "Beta" });
        expect(grid.shadowRoot?.querySelectorAll("tbody tr")).toHaveLength(1);

        grid.shadowRoot?.querySelector<HTMLInputElement>("tbody input.checkbox")?.click();
        await grid.updateComplete;
        expect(selection).toEqual({ keys: ["b"] });
        grid.shadowRoot?.querySelector<HTMLButtonElement>(".batch-actions button")?.click();
        expect(action).toMatchObject({ id: "archive", keys: ["b"] });
    });

    it("keeps virtual ranges bounded for empty, negative and oversized input", () => {
        expect(getAdminVirtualRange(0, -10, 420, 44)).toEqual({
            start: 0,
            end: 0,
            top: 0,
            bottom: 0,
        });
        expect(getAdminVirtualRange(100, 880, 420, 44, 2)).toEqual({
            start: 18,
            end: 32,
            top: 792,
            bottom: 2992,
    ...
    it("supports progress ring, loading overlay and truncated text semantics", async () => {
        const ring = document.createElement("aui-progress-ring") as HTMLElement & {
            value: number;
            max: number;
            updateComplete: Promise<boolean>;
        };
        ...
        const overlay = document.createElement("aui-loading-overlay") as HTMLElement & {
            open: boolean;
            updateComplete: Promise<boolean>;
        };
        overlay.open = true;
        const text = document.createElement("aui-truncated-text") as HTMLElement & {
            text: string;
            lines: number;
            updateComplete: Promise<boolean>;
        };
        text.text = "A long operational resource label";
        text.lines = 2;
        document.body.append(overlay, text);
        await Promise.all([ring.updateComplete, overlay.updateComplete, text.updateComplete]);
        expect(
            ring.shadowRoot?.querySelector('[role="progressbar"]')?.getAttribute("aria-valuenow"),
        ).toBe("75");
        expect(overlay.shadowRoot?.querySelector('[role="status"]')).not.toBeNull();
        expect(text.shadowRoot?.querySelector(".text")?.getAttribute("aria-label")).toContain(
            "operational",
        );
    });

    it("collapses long breadcrumbs and reveals the overflow menu", async () => {
        const breadcrumb = document.createElement("aui-breadcrumb") as HTMLElement & {
        ...
        const more = breadcrumb.shadowRoot?.querySelector<HTMLButtonElement>(".overflow-button");
