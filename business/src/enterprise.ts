...
export class AdminFormWizardElement extends AdminElement {
    ...
    protected willUpdate(changed: Map<string, unknown>): void {
        if (changed.has("steps")) {
            ...
            if (!this.active || !this.steps.some((step) => step.id === this.active))
    ...
    private canEnter(index: number): boolean {
        ...
        return (
            index <= 0 ||
            this.completed.includes(this.steps[index - 1].id) ||
            index === this.indexOf()
        );
    }
    ...
    private changeTo(id: string): void {
        ...
        if (
            !this.dispatchEvent(
                new CustomEvent("aui-wizard-before-change", {
                    bubbles: true,
                    composed: true,
                    cancelable: true,
                    detail,
    ...
    private next(): void {
        ...
        if (current < this.steps.length - 1) {
            ...
            const nextStep = this.steps
                .slice(current + 1)
                .find((_, offset) => this.canEnter(current + 1 + offset));
            if (nextStep) {
    ...
    render() {
        ...
        return html`<div class="wizard">
            <ol class="steps" @keydown=${this.onKeydown} aria-label="Form steps">
                ${this.steps.map(
                    (step, index) => html`<li>
                        ...
                        >
                            <span class="number"
                                >${this.completed.includes(step.id) ? "✓" : index + 1}</span
                            >
                            <span
                                ><span class="step-title">${step.label}</span
                                >${step.description ? html`<span class="step-description">${step.description}</span>` : null}</span
                            >
                        </button>
                    </li>`,
                )}
            ...
            <section
                class="content"
                aria-live="polite"
                aria-label=${activeStep?.label ?? "Current step"}
            >
            ...
            <div class="actions">
                <button type="button" @click=${this.previous} ?disabled=${currentIndex <= 0}>
                    ${this.previousLabel}
                </button>
                <button class="primary" type="button" @click=${this.next} ?disabled=${!activeStep}>
                    ${currentIndex >= this.steps.length - 1 ? this.finishLabel : this.nextLabel}
                </button>
            </div>
        </div>`;
    }
...
export class AdminPermissionMatrixElement extends AdminElement {
    ...
    static styles = css`
        ...
        .permission[data-value="read"] {
            border-color: var(--aui-info);
            color: var(--aui-info);
        }
        .permission[data-value="write"] {
            border-color: var(--aui-warning);
            color: var(--aui-warning);
        }
        .permission[data-value="admin"] {
            border-color: var(--aui-success);
            color: var(--aui-success);
        }
        .permission:hover:not(:disabled),
    ...
    private cycle(resourceId: string, roleId: string): void {
        ...
        const permission =
            permissionOrder[(permissionOrder.indexOf(current) + 1) % permissionOrder.length];
        this.permissions = {
            ...this.permissions,
    ...
    render() {
        if (!this.resources.length || !this.roles.length)
            return html`<div class="frame">
                <div class="state" role="status">${this.emptyLabel}</div>
            </div>`;
        return html`<div class="frame">
            <table>
                <thead>
                    <tr>
                        ...
                        ${this.roles.map((role) => html`<th scope="col">${role.label}</th>`)}
                    </tr>
                </thead>
                <tbody>
                    ${this.resources.map(
                        (resource) => html`<tr>
                            <th scope="row">
                                <span class="resource">${resource.label}</span
                                >${resource.description ? html`<span class="description">${resource.description}</span>` : null}
                            </th>
                            ${this.roles.map((role) => {
                                const value = this.value(resource.id, role.id);
                                return html`<td>
                                    <button
                                        class="permission"
                                        type="button"
                                        data-value=${value}
                                        aria-label=${`${resource.label}, ${role.label}: ${value}`}
                                        ?disabled=${this.readOnly}
                                        @click=${() => this.cycle(resource.id, role.id)}
                                    >
                                        ${value}
                                    </button>
                                </td>`;
...
export class AdminAuditLogElement extends AdminElement {
    ...
    render() {
        ...
        const entries = this.entries.filter((entry) => {
            ...
            const haystack =
                `${entry.actor} ${entry.action} ${entry.target} ${entry.details ?? ""}`.toLowerCase();
            return matchesStatus && (!query || haystack.includes(query));
        });
        const content = this.loading
            ? html`<div class="state" role="status" aria-live="polite">${this.loadingLabel}</div>`
            : this.permissionDenied
              ? html`<div class="state permission" role="status">
                    ${this.permissionDeniedLabel}
                    <div><slot name="permission"></slot></div>
                </div>`
              : this.error
                        ...
                        <tbody>
                            ${entries.map(
                                (entry) =>
                                    html`<tr>
                                        <td>${entry.time}</td>
                                        <td><span class="actor">${entry.actor}</span></td>
                                        <td>
                                            ${entry.action}${entry.details ? html`<span class="details">${entry.details}</span>` : null}
                                        </td>
                                        <td><span class="target">${entry.target}</span></td>
                                        <td>
                                            <span class=${`status status-${entry.status ?? "info"}`}
                                                >${entry.status ?? "info"}</span
                                            >
                                        </td>
                                    </tr>`,
                            )}
        ...
        return html`<div class="toolbar">
            ...
            ${this.hasMore ? html`<div class="more"><button type="button" @click=${this.loadMore} ?disabled=${this.loading}>LOAD MORE</button></div>` : null}`;
    }
}
