...
const lines = [
  ...
  "- `AdminDataResource` cache metrics are available through `getCacheStats()`, `resetCacheStats()`, `subscribeCache()` and the opt-in `cache.onEvent` hook; telemetry failures are isolated from request state.",
  "- `AdminDataResource` request telemetry is available through `subscribeTelemetry()`, `getTelemetryStats()`, `resetTelemetryStats()` and the opt-in `telemetry.onEvent` hook; request payloads stay out of telemetry unless `includeRequest` is explicitly enabled.",
  "",
];
